...
def train_model(model_ft, criterion, optimizer, lr_scheduler, num_epochs=50):
    ...
    acc_list = []
    train_logger = pd.DataFrame(columns=['epoch','train_loss','train_acc'])
    for epoch in range(num_epochs):
        ...
        running_corrects = 0
        count = 0
        steps = len(dset_loaders['train'])
        for i, (images, targets) in enumerate(dset_loaders['train'], start=1):
        # for data in dset_loaders['train']:
            inputs, labels = images, targets
            ...
            if count % 100 == 0 or outputs.size()[0] < batch_size:
                ...
                _logger.info(
                    "Epoch: [{}] [{}]/[{}]({:.2%}) "
                    "Loss: {:.4f} "
                    .format(epoch, i, steps, i / steps, loss.item()))
        ...
        acc_list.append(round(epoch_acc.data.cpu().item(), 4))
        if epoch_acc > best_acc:
            best_acc = epoch_acc
            best_model_wts = model_ft.state_dict()
        if epoch_acc > 0.999:
            break
    train_logger['epoch'] = epoch_list
    train_logger['train_loss'] = loss_list
    train_logger['train_acc'] = acc_list
    # save best model
    save_dir = data_dir + '/model'
    os.makedirs(save_dir, exist_ok=True)
    model_ft.load_state_dict(best_model_wts)
    model_out_path = save_dir + "/" + net_name + '.pth'   # 保存best model路径与名字
    torch.save(model_ft, model_out_path)
    train_logger.to_csv("./train_logger2.csv", index=0)   # train数据记录
